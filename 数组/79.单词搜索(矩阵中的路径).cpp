        ...
        //遍历board中每一个元素，判断与word中的第一个字母是否相同，如果相同则在当前位置上去搜索上下左右相邻的单元格的元素是否和当前字母的下一个字母相同，不在搜索范围内或者字母不同就返回false，当搜索的字母数等于word的长度时，也就表明在board找到了这个word。注意每次判断一个字母要标记当前位置以搜索过，以防止字母重复利用。直接更改board元素，以便在后续的判断中不会重复判断此位置，在搜索结束后改回来就可以了。
